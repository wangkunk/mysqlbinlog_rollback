#!/bin/bash
# File Name   : del_recovery.sh
# Author      : wang
# Description : delete recover according to start position.
...
mysqlbinlog --no-defaults -vv --base64-output=DECODE-ROWS --start-position=''$position'' $logname |sed -n '/### DELETE FROM `'${db}'`.`'${table}'`/,/COMMIT/p' | \
sed -n '/###/p'    | \
